...
def test_refuses_non_spend_looking_text_before_any_send() -> None:
    ...
    assert err.value.message == "Only spend-looking lines may leave the machine."
...
def test_wrong_length_classify_is_model_unavailable() -> None:
    vendor = FakeVendor(names=["Groceries", "Transport"])
    model = LanguageModel(vendor)

    with pytest.raises(AppError) as err:
        model.classify(["Test Shop 12.00"], ["Groceries"])

    assert err.value.code == "model.unavailable"
    assert err.value.message == "The language-model service is unavailable."
